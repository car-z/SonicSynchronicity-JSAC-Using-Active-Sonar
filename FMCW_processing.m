...
lambda_c = c / (Fc + B/2);  % wavelength at center frequency
k = 2 * pi / lambda_c;  % wavenumber at center frequency
...
for i = 1:num_chirps
    ...
    z(i) = range * cosd(theta);
    x(i) = range * sind(theta);
    x_smooth = smoothdata(x, 'movmean', 5);
    z_smooth = smoothdata(z, 'movmean', 5);
    range_vals(i) = range;  % optional for coloring
end

% Plot 3D motion trajectory with connected lines
figure;
set(gcf, 'Position', [500, 500, 560, 425]);  % [left, bottom, width, height]

% Plot trajectory as a colored line
cmap = jet(length(x));  % Color map for line segments
hold on;
for i = 1:length(x)-1
    plot3([x_smooth(i) x_smooth(i+1)], [time_axis(i) time_axis(i+1)], [z_smooth(i) z_smooth(i+1)], ...
          'Color', cmap(i,:), 'LineWidth', 2);
end

% Overlay scatter points
scatter3(x_smooth, time_axis(1:length(x)), z_smooth, 36, range_vals, 'filled');

xlabel('X (m)');
ylabel('Time (s)');
zlabel('Z (m)');
title('3D Motion Trajectory from AoA + Range');
colormap jet;
colorbar;
view(45, 30);
grid on;