...
%% bit sequence
bit_seq = [1 0 1 1 0 1];
bpsk_symbols = 2*bit_seq - 1;  % results in [+1 -1 +1 +1]

num_bits = length(bpsk_symbols); % only need 4 bins to carry 4 bpsk symbols
bins_to_use = active_bins(1:num_bits);  % use first 4 active subcarriers

%% IFFT to get time-domain OFDM symbol
X = zeros(1, symbol_len);
% win = hann(length(active_bins))';
win = hann(length(bins_to_use))';

%X(active_bins) = win .* (-1).^(0:length(active_bins)-1);
% X(active_bins) = (-1).^(0:length(active_bins)-1);

%X(bins_to_use) = bpsk_symbols;
X(bins_to_use) = win .* bpsk_symbols;

x_real = real(ifft(X));

%% append cyclic suffix and separation
cyclic_prefix = x_real(end-cyclic_len+1:end);
ofdm_pulse = [cyclic_prefix, x_real]';  % 168 samples total
...
audiowrite('ofdm_pulse_101101_548_prefix.wav', signal, Fs);
